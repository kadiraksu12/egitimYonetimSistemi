...
export class TakvimComponent implements OnInit {
  ...
  trackId(index: number, item: ITakvim): number {
    return item.id!;
  }

  delete(takvim: ITakvim): void {
    const modalRef = this.modalService.open(TakvimDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.takvim = takvim;
  ...
  protected onSuccess(data: ITakvim[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
